...
struct SCampaignDataFromFrame {
    ...
    std::set<ImageSizeEnum> imageSize; //set<enum.ImageSizeEnum> ;
    int64_t jumpCnt;
    ...
    std::list<int64_t> creativeIdList;
    bool isCampaignCreative  = false;
    std::map<CreativeType, std::list<int64_t>> creativeIdMap;//creativeIdMap: [CreativeIdMap];//map<enum.CreativeType, list<long>> ;
    std::map<CreativeType, std::list<CreativeAttr>> creativeAttrMap;//map<enum.CreativeType, list<CreativeAttr>> ;
    std::list<SVideo> videos;
...
struct SRankerInput {
    ...
    int64_t publisherId;
    std::list<ADSource> sourceList;//enum
    ...
    std::list<int64_t> fixedOrderVect;
    std::list<SCampaignDataFromFrame> campaignData;
    std::list<SCampaignDataFromFrame> campaignDataPreclick;
    std::list<int64_t> showedCampaignId ;
    std::string imei; //string ;
    std::string gaid; //string ;
    std::string idfa; //string ;
    bool preclick = false;
    int64_t trueNum;
    std::list<int64_t> preclickCampaignIds;
    std::string deviceBrand; //string ;
    std::list<int64_t> fixedOrderRba;
    std::map<int32_t, std::set<int64_t> > fixedOrderNormal;//map<int, set<long> > ;
    Category category;//enum.
    int64_t offset;
    std::string versionName; //string ;
    std::list<SSmartPV> smartPvList;
    std::list<SSmartPV> powerPvList;
    std::list<SSmartPV> cleverPvList;
    ...
    int32_t adTypeInt;
    std::list<int64_t> excludeIdList;
    std::list<int64_t> installIdList;
    std::list<int64_t> prePid;
    ...
    bool debugMode;
    std::list<InteractiveResourceType> resourceTypeList ;//[enum]
...
struct SRankedCampaignInfo {
	...
	std::map<CreativeType, int64_t> creativeIdMap;//   creativeIdMap: [CreativeIdMap_2]; //map<enum.CreativeType, i64> ;
    std::map<CreativeType, std::string> dco; //map<enum.CreativeType, string> ; 
...
struct SRankerOutput {
    std::list<SRankedCampaignInfo> campaignList ;
    std::list<std::string> accessLogFields;
